...
Class TemplateClass
	...
	Public Function parseNavList(str)	
		...
		dim vtype,vnum,whereStr,linkArray,lnum
		...
		for each match in matches
			...
			lnum=parseArr(labelAttrLinklist)("num") 
			if isnul(lnum) then 
				lnum=8
			else
				lnum=cint(lnum)
...
case "ico"
	...
	Public Function parseLoop(Byval str)
		dim sortArr,sortStr,sortI,labelRuleField,matches,match,labelStr,loopStr,labelArr,lnum,ltype,lsort,lorder,ltime,whereType,whereSort,orderStr,whereTime,sql,DateArray,matchesfield,loopstrTotal,i,sperStrs,spec,sperStr,aboutkey,title,lstar,contentlen,pagecontent,guidelen,guidecontent,lpassnum,paixu,tiaojian
		...
		for each match in matches
			...
			lnum = labelArr("num") : ltype = labelArr("type") : lsort = labelArr("sort") : lorder = labelArr("order") : ltime = labelArr("time") : aboutkey = labelArr("tag") : lstar=labelArr("star"):lpassnum=labelArr("passnum"):paixu=labelArr("paixu"):tiaojian=labelArr("tiaojian")
			...
			if isNul(lpassnum) then lpassnum =0  else lpassnum = cint(lpassnum)
			...
			if isNul(tiaojian) then tiaojian="all"
			tiaojian=replace(tiaojian,"$@$","=")
			if tiaojian <> "all" then 		
				whereSort=whereSort&" and "&tiaojian			
			end if
			...
			select case lorder           
				case "id" : orderStr =" order by ContentID " &paixu
				case "visits" : orderStr =" order by Visits " &paixu
				case "time" : orderStr =" order by a.AddTime " &paixu
				case "paixu" : orderStr=" order by a.ContentOrder "&paixu
				case "order" : orderStr =" order by IsTop  " &paixu& ",isrecommend  " &paixu& ",ContentOrder,a.AddTime " &paixu				
				case "istop" : orderStr =" and IsTop order by ContentOrder,a.AddTime " &paixu
				case "isrecommend" : orderStr =" and isrecommend order by ContentOrder,a.AddTime " &paixu 
				case "isimagenews" : orderStr =" and IsImageNews order by ContentOrder,a.AddTime " &paixu
				case "isheadline" : orderStr =" and IsHeadline order by ContentOrder,a.AddTime " &paixu 
				case "isfeatured" : orderStr =" and IsFeatured order by ContentOrder,a.AddTime " &paixu
			end select
			...
			if str="content" or str="news" or str="product" or str="down" or str="pic" then			
				...
				sql="select top "&lnum&" ContentID,a.SortID,a.GroupID,a.Exclusive,Title,Title2,TitleColor,IsOutLink,OutLink,Author,ContentGuide,ContentSource,ContentTag,Content,ContentStatus,IsTop,Isrecommend,IsImageNews,IsHeadline,IsFeatured,ContentOrder,IsGenerated,Visits,a.AddTime,a.ImagePath,a.IndexImage,a.DownURL,a.PageFileName,a.PageDesc,SortType,SortURL,SortFolder,SortFileName,SortName,ContentFolder,ContentFileName,b.GroupID,b.IcoImage,b.Exclusive"&sperStr&" from {prefix}Content as a,{prefix}Sort as b where a.LanguageID="&setting.languageID&"  and a.SortID=b.SortID and ContentStatus=1 and TimeStatus=0 "&whereType&whereSort&whereTime&orderStr								
			    ...
			    for each matchfield in matchesfield
					...
					if str="content" or str="aboutcontent" or str="news" or str="product" or str="down" or str="pic"  then							
						...
						select case fieldName
							...
							case "guide"
							if not isnul(rsObj("ContentGuide")) then
								guidelen = parseArr(fieldArr)("len") 
								guidecontent=rsObj("ContentGuide")
								if not isNul(guidelen) then   								
									guidelen=cint(guidelen)
									if len(guidecontent)>guidelen then guidecontent=left(guidecontent,guidelen)&"..." 
								end if	
								nloopstr = replace(nloopstr,matchfield.value,guidecontent)
							end if	
									
								'nloopstr = replace(nloopstr,matchfield.value,rsObj("ContentGuide"))	
							case "source"
					...
					elseif str="comment" then
						select case fieldName		
								...
								case "link"
									...
									commentsql="select top 1 ContentID,a.SortID,a.GroupID,a.Exclusive,Title,Title2,TitleColor,IsOutLink,OutLink,Author,ContentGuide,,ContentSource,ContentTag,Content,ContentStatus,IsTop,Isrecommend,IsImageNews,IsHeadline,IsFeatured,ContentOrder,IsGenerated,Visits,a.AddTime,a.[ImagePath],a.IndexImage,a.DownURL,a.PageFileName,a.PageDesc,SortType,SortURL,SortFolder,SortFileName,SortName,ContentFolder,ContentFileName,b.GroupID,b.Exclusive from {prefix}Content as a,{prefix}Sort as b where ContentID="&DateArray(1,i)
									'die commentsql
	...
	Public Function parseList(typeIds,currentPage,pageListType,keys,showType)	
	    dim lenPagelist,TypeId,strPagelist,lsize,rsObj,labelRuleField,labelRulePagelist,matches,match,labelStr,loopStr,labelArr,lorder,orderStr,sql,matchesfield,sperStrs,spec,sperStr,title,aboutkey,guidelen,guidecontent
				...
				if isNul(keys) then					
					if not isnul(aboutkey) then		
						...
						if not isnul(aboutkey) then 		
							...
							aboutkey=replace(aboutkey,"%'","}%'")
							aboutkey=replace(aboutkey,"'%","'%{") 
							sql="select ContentID,a.SortID,a.GroupID,a.Exclusive,Title,Title2,TitleColor,IsOutLink,OutLink,Author,ContentGuide,ContentSource,ContentTag,Content,ContentStatus,IsTop,Isrecommend,IsImageNews,IsHeadline,IsFeatured,ContentOrder,IsGenerated,Visits,a.AddTime,a.[ImagePath],a.IndexImage,a.DownURL,a.PageFileName,a.PageDesc,SortType,SortURL,SortFolder,SortFileName,SortName,ContentFolder,ContentFileName,b.GroupID,b.Exclusive,b.GroupID "&sperStr&" from {prefix}Content as a,{prefix}Sort as b where a.LanguageID="&setting.languageID&" and a.SortID=b.SortID and ContentStatus=1 and TimeStatus=0  and "&aboutkey&orderStr
							'die sql
						end if						
					else
						...
						select case fieldName
							...
							case "guide"
								if not isnul(rsObj("ContentGuide")) then
									...
									guidelen = parseArr(fieldArr)("len") 
									guidecontent=filterStr(decodeHtml(rsObj("Content")),"html")
									if not isNul(guidelen) then   								
										guidelen=cint(guidelen)
										if len(guidecontent)>guidelen then guidecontent=left(guidecontent,guidelen)&"..." 
									end if	
